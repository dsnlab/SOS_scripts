---
title: "SOS Biologiacal data visualisation: IL10"
...
First, we import the SOS_hormones_cleaned.csv file and remove participants who do not have IL10 data at all 4 time points. 
```{r}
#import cleaned bioloigcal dataset:
setwd("~/Documents/8_Byrne_work/SOS_Bioloigical_data_cleaned")
SOS_IL10 <- read.csv('SOS_cytokines_cleaned.csv')
SOS_IL10$SampleID <- as.character(SOS_IL10$SampleID)

SOS_IL10 <- SOS_IL10[4:8]
SOS_IL10 <- (SOS_IL10 %>% arrange(SampleID,Time))

#Remove participants who do not have all 3 timepoints:
SOS_IL10_T1 = SOS_IL10 %>% filter(Time == "1")
SOS_IL10_T2 = SOS_IL10 %>% filter(Time == "2")
SOS_IL10_T3 = SOS_IL10 %>% filter(Time == "3")
SOS_IL10_T4 = SOS_IL10 %>% filter(Time == "4")

Merge1 <-merge(SOS_IL10_T1, SOS_IL10_T2, by = "SampleID", all = FALSE, all.x = TRUE)
Merge2 <-merge(Merge1, SOS_IL10_T3, by = "SampleID", all = FALSE, all.x = TRUE)
SOS_IL10_reshaped <-merge(Merge2, SOS_IL10_T4, by = "SampleID", all = FALSE, all.x = TRUE)

SOS_IL10_all_timepoints <- na.omit(SOS_IL10_reshaped)

#Merge to now include only the people who have all 4 timepoints:
SOS_IL10_final <- merge(SOS_IL10, SOS_IL10_all_timepoints, by = "SampleID", all = FALSE)
SOS_IL10_final <- SOS_IL10_final[1:5]
colnames(SOS_IL10_final) <- c("SampleID", "Time","IL10_Ave", "IL10_log", "IL10_nontran_win")
SOS_IL10_final <- (SOS_IL10_final %>% arrange(SampleID,Time))

#Clear variables no longer needed:
rm(SOS_IL10_T1, SOS_IL10_T2,SOS_IL10_T3,SOS_IL10_T4, Merge1, Merge2, SOS_IL10_reshaped, SOS_IL10_all_timepoints)

total <- unique(SOS_IL10$SampleID)
total_keep <- unique(SOS_IL10_final$SampleID)
...
```{r}
All_participants_plot <- ggplot(SOS_IL10_final, aes(x = Time, y = IL10_log, group = SampleID, colour = SampleID)) +
                          geom_line()
...
for (i in unique(SOS_IL10_final$SampleID)) {
    idx <- which(SOS_IL10_final$SampleID==i, arr.ind=TRUE)
    #Get participants peak timepoint
    PeakTime[i] <- which.max(SOS_IL10_final$IL10_log[idx])
    #Get SampleID
    PeakTime_id[i] <- i
...
SOS_IL10_final <-merge(SOS_IL10_final, Peak_time_all, by = "SampleID", all = TRUE)
rm(Peak_time_all)
SOS_IL10_final$PeakTime <- as.factor(SOS_IL10_final$PeakTime)

```

We have split the data according to where the participant's maximum IL10 (log) response occured; at time point 1, 2, 3 or 4. 
We can then plot the change over time, depending on the group variable "Peak Time":
```{r}
Overall <- ggline(SOS_IL10_final, "Time", "IL10_log", 
           group = "PeakTime",
...
```{r}
(summary(SOS_IL10_final$PeakTime)/4)
```
...
PeakTime1 <- SOS_IL10_final %>% filter(PeakTime == "1") 
PeakTime2 <- SOS_IL10_final %>% filter(PeakTime == "2") 
PeakTime3 <- SOS_IL10_final %>% filter(PeakTime == "3") 
PeakTime4 <- SOS_IL10_final %>% filter(PeakTime == "4") 
```

Now for more detail, within each "Peak time" group, we can see how individual subjects reacted:
```{r}
PT1 <- ggplot(data = PeakTime1,aes(x = Time, y = IL10_log, group = SampleID, colour = SampleID)) +
       geom_line()+
...
```{r}
PT2 <- ggplot(data = PeakTime2,aes(x = Time, y = IL10_log, group = SampleID, colour = SampleID)) + 
      geom_line()+
      stat_summary(aes(group = 1), geom = "line", fun = mean, size = 1)
PT2
```
...
```{r}
PT3 <- ggplot(data = PeakTime3,aes(x = Time, y = IL10_log, group = SampleID, colour = SampleID)) + 
      geom_line()+
     stat_summary(aes(group = 1), geom = "line", fun = mean, size = 1)
PT3
```
...
```{r}
PT4 <- ggplot(data = PeakTime4,aes(x = Time, y = IL10_log, group = SampleID, colour = SampleID)) + 
        geom_line()+
        stat_summary(aes(group = 1), geom = "line", fun = mean, size = 1)
PT4
```
...
RepondersvNoResponders_plot <-ggline(SOS_IL10_final, "Time", "IL10_log", 
                               add = c("mean_sd", "jitter"),
                               color = "Responders")   
...
```{r}
(summary(SOS_IL10_final$Responders)/4)
```
...
Time_point_1 <- SOS_IL10_final[1:7]
Time_point_1 = Time_point_1 %>% filter(Time == "1")

compare_means(IL10_log ~ Responders, data = Time_point_1,
              method = "t.test")
```
...
for (i in as.character(SOS_IL10_final$SampleID)) {
  idx <- which(SOS_IL10_final$SampleID==i, arr.ind=TRUE)
  SampleID_current <- i
  baseline <- SOS_IL10_final$IL10_log[idx[1]]
    for (t in 1:4){
    Normalised_values[idx[t]] <- (SOS_IL10_final$IL10_log[idx[t]] - baseline)
...
names(Normalised_all)[1]<- "BL_Norm_IL10_log"
names(Normalised_all)[2]<- "SampleID"

#Merge with SOS_Cortisol_final and remove unecessary extra SampleID column:
SOS_IL10_final <- cbind(SOS_IL10_final, Normalised_all)
SOS_IL10_final <- SOS_IL10_final[1:8]
...
```{r}
Relative_to_baseline <- ggline(SOS_IL10_final, "Time", "BL_Norm_IL10_log", 
           group = "PeakTime",
           add = c("mean_sd", "jitter"),
           color = "PeakTime")
...
```{r}
Relative_to_baseline_resp <- ggline(SOS_IL10_final, "Time", "BL_Norm_IL10_log", 
           group = "Responders",
           size = 0.5,
           add = c("mean_sd", "jitter"),
           color = "Responders")
...
#Do a t-test to compare whether non-responders(0) had a higher T1 than responders(1)
Responders <- SOS_IL10_final[1:8]
Responders = Responders %>% filter(Responders == 1)

compare_means(BL_Norm_IL10_log ~ Time, data = Responders,
              method = "anova")
...
compare_means(BL_Norm_IL10_log ~ Time, data = Responders, paired = TRUE,
              method = "t.test")
```
...
for (i in unique(SOS_IL10_final$SampleID)) {
    ...
    AUC_list[i] <- auc(
                SOS_IL10_final$Time[idx],
                SOS_IL10_final$BL_Norm_IL10_log[idx],
...
#Rename columns for merge (SampleID, also helps for checking that the rows have matched appropriately)
names(AUC_all)[1]<- "AUC_BL_Norm_IL10_log"
names(AUC_all)[2]<- "SampleID"

#Merge with SOS_Cortisol_final and remove unecessary extra SampleID column:
SOS_IL10_final <-merge(SOS_IL10_final, AUC_all, by = "SampleID", all = TRUE)

#Clear variables no longer needed:
rm(AUC_all, i, idx)
#Update Responders dataframe to include AUC 
Responders <- SOS_IL10_final[1:9]
Responders = Responders %>% filter(Responders == 1)
...
```{r}
#Plot AUC value for
AUC_plot <- ggplot(SOS_IL10_final, aes(Responders, AUC_BL_Norm_IL10_log, colour=Responders)) +
    geom_violin() +
...
We can see that the greatest change in IL10 level occurs between T1 and T2. We are therefore interested to quantify the magnitude of this change per participant. If participant's change is +ve (meaning an increase from T1 --> T2) = 1. If participant's change is -ve (meaning a decrease from T1 --> T2) = 0: 
...
for (i in as.character(SOS_IL10_final$SampleID)){
    idx <- which(SOS_IL10_final$SampleID==i, arr.ind=TRUE)
    T1T2_id[i] <-i
    T1T2_list[i] <-(SOS_IL10_final$IL10_log[idx[2]] - SOS_IL10_final$IL10_log[idx[1]])
     if (T1T2_list[i] > "0"){ 
      T1T2_direction[i] <- "1"
      } else {
      T1T2_direction[i] <- "0"
...
#Merge with SOS_IL10_final and remove unecessary extra SampleID column:
SOS_IL10_final <-merge(SOS_IL10_final, T1T2_all, by = "SampleID", all = TRUE)
rm(T1T2_all)

(summary(SOS_IL10_final$Change_T1_T2_Direction)/4)
```

```{r}
SOS_IL10_final$Change_T1_T2_Direction <- as.factor(SOS_IL10_final$Change_T1_T2_Direction)
```

```{r}
ChangeT1T2 <- ggplot(SOS_IL10_final, aes(Change_T1_T2_Direction, Change_T1_T2, colour=Responders)) +
    geom_violin() +
    geom_boxplot(width=0.2, position = "dodge2") + 
    stat_summary(fun.data=mean_sdl, 
                 geom="pointrange") +
...
```{r}
Resp_pos <- count(which(SOS_IL10_final$Responders == 1 & SOS_IL10_final$Change_T1_T2_Direction == 1))
(nrow(Resp_pos)/4)
```

How many people are responders who have a negative change: 
```{r}
Resp_neg <- count(which(SOS_IL10_final$Responders == 1 & SOS_IL10_final$Change_T1_T2_Direction == 0))
(nrow(Resp_neg)/4)
```
How many people are non-responders who have a positive change: 
```{r}
Non_resp_pos <- count(which(SOS_IL10_final$Responders == 0 & SOS_IL10_final$Change_T1_T2_Direction == 1))
(nrow(Non_resp_pos)/4)
```

How many people are non-responders who have a negative change: 
```{r}
Non_resp_neg <- count(which(SOS_IL10_final$Responders == 0 & SOS_IL10_final$Change_T1_T2_Direction == 0))
(nrow(Non_resp_neg)/4)
```
...
#Rename columns for merge (SampleID, also helps for checking that the rows have matched appropriately)
names(AUC_all)[1]<- "AUC_BL_Norm_IL10_log_T1_T2"
names(AUC_all)[2]<- "SampleID"
...
AUC_plot <- ggplot(SOS_IL10_final, aes(Change_T1_T2_Direction, AUC_BL_Norm_IL10_log_T1_T2, colour=Change_T1_T2_Direction)) +
    geom_violin() +
    geom_boxplot(width=0.2, position = "dodge2") + 
    stat_summary(fun.data=mean_sdl, 
                 geom="pointrange") +
