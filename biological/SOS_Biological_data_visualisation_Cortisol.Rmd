---
title: "SOS Biologiacal data visualisation: CORTISOL"
...
SOS_Cortisol <- SOS_hormones_cleaned[,c("SampleID", "Time","Cortisol_ug_dL", "Cortisol_log", "Cortisol_log")]

#Remove participants who do not have all 3 timepoints:
SOS_Cortisol_T1 = SOS_Cortisol %>% filter(Time == "1")
SOS_Cortisol_T2 = SOS_Cortisol %>% filter(Time == "2")
SOS_Cortisol_T3 = SOS_Cortisol %>% filter(Time == "3")
SOS_Cortisol_T4 = SOS_Cortisol %>% filter(Time == "4")

Merge1 <-merge(SOS_Cortisol_T1, SOS_Cortisol_T2, by = "SampleID", all = FALSE, all.x = TRUE)
Merge2 <-merge(Merge1, SOS_Cortisol_T3, by = "SampleID", all = FALSE, all.x = TRUE)
SOS_Cortisol_reshaped <-merge(Merge2, SOS_Cortisol_T4, by = "SampleID", all = FALSE, all.x = TRUE)

SOS_Cortisol_all_timepoints <- na.omit(SOS_Cortisol_reshaped)

#Merge to now include only the people who have all 4 timepoints:
SOS_Cortisol_final <- merge(SOS_Cortisol, SOS_Cortisol_all_timepoints, by = "SampleID", all = FALSE)
SOS_Cortisol_final <- SOS_Cortisol_final[1:5]
SOS_Cortisol_final <- (SOS_Cortisol_final %>% arrange(SampleID,Time))

total <- unique(SOS_Cortisol$SampleID)
total_keep <- unique(SOS_Cortisol_final$SampleID)
...
Plot each participants response (Cortisol_log) over time:
```{r}

#Clear variables no longer needed:
rm(SOS_Cortisol_T1, SOS_Cortisol_T2,SOS_Cortisol_T3,SOS_Cortisol_T4, Merge1, Merge2, SOS_Cortisol_reshaped, SOS_Cortisol_all_timepoints, SOS_Cortisol)

All_participants_plot <- ggplot(SOS_Cortisol_final, aes(x = Time, y = Cortisol_log, group = SampleID, colour = SampleID)) +
                          geom_line()
...
for (i in unique(SOS_Cortisol_final$SampleID)) {
    idx <- which(SOS_Cortisol_final$SampleID==i, arr.ind=TRUE)
    #Get participants peak timepoint
    PeakTime[i] <- which.max(SOS_Cortisol_final$Cortisol_log[idx])
    #Get SampleID
    PeakTime_id[i] <- i
...
SOS_Cortisol_final <-merge(SOS_Cortisol_final, Peak_time_all, by = "SampleID", all = TRUE)
rm(Peak_time_all)
SOS_Cortisol_final$PeakTime <- as.factor(SOS_Cortisol_final$PeakTime)
...
```{r}
Overall <- ggline(SOS_Cortisol_final, "Time", "Cortisol_log", 
           group = "PeakTime",
...
```
```{r}
summary(SOS_Cortisol_final)
```
The number of subjects per "Peak Time" group:
```{r}
(summary(SOS_Cortisol_final$PeakTime)/4)
```
...
PeakTime1 <- SOS_Cortisol_final %>% filter(PeakTime == "1") 
PeakTime2 <- SOS_Cortisol_final %>% filter(PeakTime == "2") 
PeakTime3 <- SOS_Cortisol_final %>% filter(PeakTime == "3") 
PeakTime4 <- SOS_Cortisol_final %>% filter(PeakTime == "4") 
```

Now for more detail, within each "Peak time" group, we can see how individual subjects reacted:
```{r}
PT1 <- ggplot(data = PeakTime1,aes(x = Time, y = Cortisol_log, group = SampleID, colour = SampleID)) +
       geom_line()+
...
```{r}
PT2 <- ggplot(data = PeakTime2,aes(x = Time, y = Cortisol_log, group = SampleID, colour = SampleID)) + 
      geom_line()+
      stat_summary(aes(group = 1), geom = "line", fun = mean, size = 1)
PT2
```
...
```{r}
PT3 <- ggplot(data = PeakTime3,aes(x = Time, y = Cortisol_log, group = SampleID, colour = SampleID)) + 
      geom_line()+
     stat_summary(aes(group = 1), geom = "line", fun = mean, size = 1)
PT3
```
...
```{r}
PT4 <- ggplot(data = PeakTime4,aes(x = Time, y = Cortisol_log, group = SampleID, colour = SampleID)) + 
        geom_line()+
        stat_summary(aes(group = 1), geom = "line", fun = mean, size = 1)
PT4
```
...
RepondersvNoResponders_plot <-ggline(SOS_Cortisol_final, "Time", "Cortisol_log", 
                               add = c("mean_sd", "jitter"),
                               color = "Responders")   
...
```{r}
(summary(SOS_Cortisol_final$Responders)/4)
```
...
Time_point_1 <- SOS_Cortisol_final[1:7]
Time_point_1 = Time_point_1 %>% filter(Time == "1")

compare_means(Cortisol_log ~ Responders, data = Time_point_1,
              method = "t.test")
```
...
for (i in as.character(SOS_Cortisol_final$SampleID)) {
  idx <- which(SOS_Cortisol_final$SampleID==i, arr.ind=TRUE)
  SampleID_current <- i
  baseline <- SOS_Cortisol_final$Cortisol_log[idx[1]]
    for (t in 1:4){
    Normalised_values[idx[t]] <- (SOS_Cortisol_final$Cortisol_log[idx[t]] - baseline)
...
names(Normalised_all)[1]<- "BL_Norm_Cortisol_log"
names(Normalised_all)[2]<- "SampleID"

#Merge with SOS_Cortisol_final and remove unecessary extra SampleID column:
SOS_Cortisol_final <- cbind(SOS_Cortisol_final, Normalised_all)
SOS_Cortisol_final <- SOS_Cortisol_final[1:8]
...
```{r}
Relative_to_baseline <- ggline(SOS_Cortisol_final, "Time", "BL_Norm_Cortisol_log", 
           group = "PeakTime",
           add = c("mean_sd", "jitter"),
           color = "PeakTime")
...
```{r}
Relative_to_baseline_resp <- ggline(SOS_Cortisol_final, "Time", "BL_Norm_Cortisol_log", 
           group = "Responders",
           size = 0.5,
           add = c("mean_sd", "jitter"),
           color = "Responders")
...
#Test effect of time on response
Responders <- SOS_Cortisol_final[1:8]
Responders = Responders %>% filter(Responders == 1)

compare_means(BL_Norm_Cortisol_log ~ Time, data = Responders,
              method = "anova")

```
```{r}
#Follow up repeated measures t-tests
compare_means(BL_Norm_Cortisol_log ~ Time, data = Responders, paired = TRUE,
              method = "t.test")
```
...
for (i in unique(SOS_Cortisol_final$SampleID)) {
    ...
    AUC_list[i] <- auc(
                SOS_Cortisol_final$Time[idx],
                SOS_Cortisol_final$BL_Norm_Cortisol_log[idx],
...
#Rename columns for merge (SampleID, also helps for checking that the rows have matched appropriately)
names(AUC_all)[1]<- "AUC_BL_Norm_Cortisol_log"
names(AUC_all)[2]<- "SampleID"

#Merge with SOS_Cortisol_final and remove unecessary extra SampleID column:
SOS_Cortisol_final <-merge(SOS_Cortisol_final, AUC_all, by = "SampleID", all = TRUE)

#Clear variables no longer needed:
rm(i, idx, AUC_all)

#Update Responders dataframe to include AUC 
Responders <- SOS_Cortisol_final[1:9]
Responders = Responders %>% filter(Responders == 1)
...
```{r}
#Plot AUC value for
AUC_plot <- ggplot(SOS_Cortisol_final, aes(Responders, AUC_BL_Norm_Cortisol_log, colour=Responders)) +
    geom_violin() +
...
We can see that the greatest change in Cortisol level occurs between T2 and T3. We are therefore interested to quantify the magnitude of this change per participant. If participant's change is +ve (meaning an increase from T2 --> T3) = 1. If participant's change is -ve (meaning a decrease from T2 --> T3) = 0: 
...
for (i in as.character(SOS_Cortisol_final$SampleID)){
    idx <- which(SOS_Cortisol_final$SampleID==i, arr.ind=TRUE)
    T2T3_id[i] <-i
    T2T3_list[i] <-(SOS_Cortisol_final$Cortisol_log[idx[3]] - SOS_Cortisol_final$Cortisol_log[idx[2]])
     if (T2T3_list[i] > "0"){ 
      T2T3_direction[i] <- "1"
      } else {
      T2T3_direction[i] <- "0"
...
#Merge with SOS_Cortisol_final and remove unecessary extra SampleID column:
SOS_Cortisol_final <-merge(SOS_Cortisol_final, T2T3_all, by = "SampleID", all = TRUE)
rm(T2T3_all)

(summary(SOS_Cortisol_final$Change_T2_T3_Direction)/4)
```

```{r}
SOS_Cortisol_final$Change_T2_T3_Direction <- as.factor(SOS_Cortisol_final$Change_T2_T3_Direction)
```

```{r}
ChangeT2T3 <- ggplot(SOS_Cortisol_final, aes(Change_T2_T3_Direction, Change_T2_T3, colour=Responders)) +
    geom_violin() +
    geom_boxplot(width=0.2, position = "dodge2") + 
    stat_summary(fun.data=mean_sdl, 
                 geom="pointrange") +
...
```{r}
Resp_pos <- count(which(SOS_Cortisol_final$Responders == 1 & SOS_Cortisol_final$Change_T2_T3_Direction == 1))
(nrow(Resp_pos)/4)
```

How many people are responders who have a negative change: 
```{r}
Resp_neg <- count(which(SOS_Cortisol_final$Responders == 1 & SOS_Cortisol_final$Change_T2_T3_Direction == 0))
(nrow(Resp_neg)/4)
```
How many people are non-responders who have a positive change: 
```{r}
Non_resp_pos <- count(which(SOS_Cortisol_final$Responders == 0 & SOS_Cortisol_final$Change_T2_T3_Direction == 1))
(nrow(Non_resp_pos)/4)
```

How many people are non-responders who have a negative change: 
```{r}
Non_resp_neg <- count(which(SOS_Cortisol_final$Responders == 0 & SOS_Cortisol_final$Change_T2_T3_Direction == 0))
(nrow(Non_resp_neg)/4)
```
...
#Rename columns for merge (SampleID, also helps for checking that the rows have matched appropriately)
names(AUC_all)[1]<- "AUC_BL_Norm_Cortisol_log_T2_T3"
names(AUC_all)[2]<- "SampleID"
...
AUC_plot <- ggplot(SOS_Cortisol_final, aes(Change_T2_T3_Direction, AUC_BL_Norm_Cortisol_log_T2_T3, colour=Change_T2_T3_Direction)) +
    geom_violin() +
    geom_boxplot(width=0.2, position = "dodge2") + 
    stat_summary(fun.data=mean_sdl, 
                 geom="pointrange") +
